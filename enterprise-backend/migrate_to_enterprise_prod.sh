...
ssh root@$SERVER_IP "mysql -u enterprise_user -penterprise_password_2024 -e 'USE enterprise_prod; SHOW TABLES;'"

# 2. 备份enterprise_db数据
echo ""
echo "💾 备份enterprise_db数据..."
ssh root@$SERVER_IP "mysqldump -u enterprise_user -penterprise_password_2024 enterprise_db > /tmp/enterprise_db_backup.sql"

# 3. 清空enterprise_prod数据库
echo ""
echo "🧹 清空enterprise_prod数据库..."
ssh root@$SERVER_IP "mysql -u enterprise_user -penterprise_password_2024 -e 'DROP DATABASE IF EXISTS enterprise_prod;'"
ssh root@$SERVER_IP "mysql -u enterprise_user -penterprise_password_2024 -e 'CREATE DATABASE enterprise_prod CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;'"

# 4. 将enterprise_db的数据导入到enterprise_prod
echo ""
echo "📥 导入数据到enterprise_prod..."
ssh root@$SERVER_IP "mysql -u enterprise_user -penterprise_password_2024 enterprise_prod < /tmp/enterprise_db_backup.sql"
...
ssh root@$SERVER_IP "mysql -u enterprise_user -penterprise_password_2024 -e 'USE enterprise_prod; SELECT id, username, email, role, status FROM users;'"
...
ssh root@$SERVER_IP "mysql -u enterprise_user -penterprise_password_2024 -e \"USE enterprise_prod; UPDATE users SET password_hash = '$NEW_HASH' WHERE role = 'admin';\""

# 7. 删除enterprise_db数据库
echo ""
echo "🗑️  删除enterprise_db数据库..."
ssh root@$SERVER_IP "mysql -u enterprise_user -penterprise_password_2024 -e 'DROP DATABASE enterprise_db;'"
