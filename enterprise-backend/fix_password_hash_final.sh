...
sshpass -p "$SSH_PASSWORD" ssh -o StrictHostKeyChecking=no root@$SERVER_IP "mysql -u enterprise_user -penterprise_password_2024 -e 'USE enterprise_prod; DESCRIBE users;'"

# 2. 修改password_hash字段长度
echo ""
echo "🔄 修改password_hash字段长度..."
sshpass -p "$SSH_PASSWORD" ssh -o StrictHostKeyChecking=no root@$SERVER_IP "mysql -u enterprise_user -penterprise_password_2024 -e 'USE enterprise_prod; ALTER TABLE users MODIFY COLUMN password_hash VARCHAR(255);'"
