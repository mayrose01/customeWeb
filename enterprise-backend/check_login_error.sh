...
echo ""
echo "🗄️  检查数据库连接..."
ssh root@$SERVER_IP "mysql -u enterprise_user -penterprise_password_2024 -e 'USE enterprise_prod; SELECT id, username, email, role, status FROM users WHERE username = \"admin\";'"
...
echo ""
echo "🔍 检查用户密码哈希..."
ssh root@$SERVER_IP "mysql -u enterprise_user -penterprise_password_2024 -e 'USE enterprise_prod; SELECT username, password_hash FROM users WHERE username = \"admin\";'"
