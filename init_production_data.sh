...
mysql -u enterprise_user -penterprise_password_2024 enterprise_prod << 'MYSQL_EOF'
...
verify_data() {
    ...
    sshpass -p "$SERVER_PASSWORD" ssh -o StrictHostKeyChecking=no root@$SERVER_IP << 'EOF'
echo "=== 公司信息 ==="
mysql -u enterprise_user -penterprise_password_2024 -e 'USE enterprise_prod; SELECT name, email, phone FROM company_info;'

echo "=== 产品分类 ==="
mysql -u enterprise_user -penterprise_password_2024 -e 'USE enterprise_prod; SELECT name, description FROM categories;'

echo "=== 示例产品 ==="
mysql -u enterprise_user -penterprise_password_2024 -e 'USE enterprise_prod; SELECT title, model FROM products LIMIT 3;'

echo "=== 轮播图 ==="
mysql -u enterprise_user -penterprise_password_2024 -e 'USE enterprise_prod; SELECT caption, description FROM carousel_images;'

echo "=== 业务板块 ==="
mysql -u enterprise_user -penterprise_password_2024 -e 'USE enterprise_prod; SELECT name, description FROM services;'
EOF
}
